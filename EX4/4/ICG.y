...
%type<str> statement cond_statement assgn_statement expr term factor NUM ASSIGN_OP ARITH_OP ARITH_OP_2 COMPARISON_OP program ID AND OR NOT
%%
program: statement {printf("%s",$1);} program | statement {printf("%s",$1);};
statement : assgn_statement EOS {sprintf($$,"%s\n",$1);};
| cond_statement {sprintf($$,"%s\n",$1);}
| loop_statement {sprintf($$,"$s\n",$1);}
| '{' program '}' {sprintf($$,"{%s}\n",$2);}
;
